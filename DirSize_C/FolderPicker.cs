...
namespace DirSize_C
{
    class FolderPicker
    {
        ...
        // Use like this.   SelectedFolder = FileSelect.GetFolder(this);
        internal string GetFolder(Form1 me) // internal string GetFolder(Form1 me)
        {
            var F = new FolderPicker
            {
                InputPath = ""   // Don't touch. The last path will be saved.  --> not needed. @"c:\windows\system32";
            };
            if (F.ShowDialog(me.Handle) == true)
            {
                return F.ResultPath;
            }
            return "";
